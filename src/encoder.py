...
class DropPath(nn.Module):
    ...
    Linearly increasing drop probability across layers helps deeper
    transformers generalize better.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Create random tensor with shape (batch, 1, 1, ...) for broadcasting
...
class TransformerEncoderLayerWithDropPath(nn.Module):
    """Transformer encoder layer with stochastic depth on residual connections."""
    ...
    def forward(self, src, src_key_padding_mask=None):
        if self.norm_first:
            src = src + self.drop_path(self._sa_block(self.norm1(src), src_key_padding_mask))
            src = src + self.drop_path(self._ff_block(self.norm2(src)))
        else:
            src = self.norm1(src + self.drop_path(self._sa_block(src, src_key_padding_mask)))
            src = self.norm2(src + self.drop_path(self._ff_block(src)))
        return src
